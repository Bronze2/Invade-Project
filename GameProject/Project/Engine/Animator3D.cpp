...
void CAnimator3D::UpdateData()
{
    if (!m_bFinalMatUpdate) {
        ...
        if (GetObj()->GetScript<CPlayerScript>() != nullptr && CSceneMgr::GetInst()->GetCurScene()->GetCurScene() == SCENE_TYPE::INGAME
            && GetObj()->GetScript<CPlayerScript>()->m_GetId() == Network::GetInst()->getMainClient().id) {
            fIsPlayer = 1.f;
...
void CAnimator3D::FinalUpdate()
{
    ...
    {
        m_fBlendFrame += 0.3f;
        if (nullptr != GetObj()->GetScript<CPlayerScript>()) {
            if (m_iCurClip == 5) {
                m_fAnimationSpeed = 0.9f;
            }
            else {
...
CAnimator3D::CAnimator3D() :CComponent(COMPONENT_TYPE::ANIMATOR3D), m_iCurClip(0), m_dCurTime(0.f), m_iFrameCount(30), m_fRatio(0.f), m_pBoneFinalMat(nullptr), m_bFinalMatUpdate(false), m_bBlendAnimation(false), m_fBlendFrame(0.f), m_fBlendMaxFrame(1.f), m_fAnimationSpeed(1.f)
{
