	...
	for (int i = (UINT)INGAME_LAYER::BLUE; i < (UINT)INGAME_LAYER::RED + 1; ++i) {
		...
		for (int j = 0; j < vecObj.size(); ++j) {
			...
			if (L"Minion" == vecObj[j]->GetName())
				continue;
			if (nullptr != vecObj[j]->GetScript<CPlayerColScript>()) {
				if ((UINT)vecObj[j]->GetScript<CPlayerColScript>()->GetPlayer()->GetScript<CPlayerScript>()->GetCamp() == (UINT)m_eState)
				{
					bool bTrue = CCollisionMgr::GetInst()->CollisionCubeMatrix(MatColWorld, vecObj[j]->Collider3D()->GetColliderWorldMatNotify());
					if (bTrue) {
						vecObj[j]->GetScript<CPlayerColScript>()->GetPlayer()->GetScript<CPlayerScript>()->SetCanAttack(true);
					}
					else {
						vecObj[j]->GetScript<CPlayerColScript>()->GetPlayer()->GetScript<CPlayerScript>()->SetCanAttack(false);
					}
				}
			}
		}
