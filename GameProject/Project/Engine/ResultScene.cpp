...
//-----------------------------------------------------------------------------------------------
	...
	for (int i = 0; i < 8; i++) {
		...
		if (i < 4) {
			...
			pPlayerObj->GetScript<CPlayerScript>()->SetResultAnim(true);		// 승리팀
		}
		else {
			FindLayer(L"Red")->AddGameObject(pPlayerObj);
			pPlayerObj->GetScript<CPlayerScript>()->SetResultAnim(false);		// 패배팀
			pPlayerObj->Transform()->SetLocalPos(Vec3(100.f + (i - 4) * 100.f, -70.f, 580.f));	// 패배팀 애니메이션때문에 앞으로 좀 더 땡김
		}
