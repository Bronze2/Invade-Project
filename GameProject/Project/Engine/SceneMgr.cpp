...
void CSceneMgr::Init()
{
 ...
 //   pObject->AddComponent(new CPlayerScript);
    ...
    //   pMeshData->Save(pMeshData->GetPath())
    //   Ptr<CMeshData> pMeshData = CResMgr::GetInst()->LoadFBX(L"FBX\\ArcherPinkGold.fbx");
    //   pMeshData->Save(pMeshData->GetPath());
    ...
    cout << "생성된 Player 개수 씬 : " << m_pCurScene->FindLayer(L"Monster")->GetParentObj().size() << endl;;
...
void CSceneMgr::EnterClient(int id ,float x, float y, float z)
{
    ...
    cout << "생성된 Player 개수 : " << m_pCurScene->FindLayer(L"Monster")->GetParentObj().size() << endl;
...
void CSceneMgr::setMainClient(int id, float x, float y, float z)
{
    ...
    m_pCurScene->FindLayer(L"Monster")->GetGameObjectById(id)->AddChild(m_pCurScene->FindLayer(L"Default")->GetGameObjectById(0));
