...
void CSceneMgr::Init()
{
    ...
    Skill->bUse = false;
    Skill->Sum = 50.f;
    Skill->pTex = CResMgr::GetInst()->Load<CTexture>(L"Water0", L"Texture\\Skill\\Water1.png").GetPointer();
    ...
    Skill->bUse = false;
    Skill->Sum = 100.f;
    Skill->pTex = CResMgr::GetInst()->Load<CTexture>(L"Water1", L"Texture\\Skill\\Water2.png").GetPointer();
    ...
    Skill->DotDamage = 0.f;
    Skill->bUse = false;
    Skill->pTex = CResMgr::GetInst()->Load<CTexture>(L"Wind1", L"Texture\\Skill\\Wind1.png").GetPointer();
    CSkillMgr::GetInst()->AddSkill(Skill->Code, Skill);
    ...
    Skill->DotDamage = 0.f;
    Skill->bUse = false;
    Skill->pTex = CResMgr::GetInst()->Load<CTexture>(L"Wind2", L"Texture\\Skill\\Wind2.png").GetPointer();
    CSkillMgr::GetInst()->AddSkill(Skill->Code, Skill);
    ...
    Skill->DotDamage = 0.f;
    Skill->bUse = false;
    Skill->pTex = CResMgr::GetInst()->Load<CTexture>(L"Dark1", L"Texture\\Skill\\Dark1.png").GetPointer();
    CSkillMgr::GetInst()->AddSkill(Skill->Code, Skill);
    ...
    Skill->DotDamage = 0.f;
    Skill->bUse = false;
    Skill->pTex = CResMgr::GetInst()->Load<CTexture>(L"Dark2", L"Texture\\Skill\\Dark2.png").GetPointer();
    CSkillMgr::GetInst()->AddSkill(Skill->Code, Skill);
    ...
    Skill->DotDamage = 0.f;
    Skill->bUse = false;
    Skill->pTex = CResMgr::GetInst()->Load<CTexture>(L"Thunder1", L"Texture\\Skill\\Thunder1.png").GetPointer();
    CSkillMgr::GetInst()->AddSkill(Skill->Code, Skill);
    ...
    Skill->Sum = -72.f;
    Skill->pTex = CResMgr::GetInst()->Load<CTexture>(L"Thunder2", L"Texture\\Skill\\Thunder2.png").GetPointer();
    CSkillMgr::GetInst()->AddSkill(Skill->Code, Skill);
    ...
    Skill->Sum = -25.f;
    Skill->bUse = false;
    Skill->pTex = CResMgr::GetInst()->Load<CTexture>(L"Fire1", L"Texture\\Skill\\Fire1.png").GetPointer();
    CSkillMgr::GetInst()->AddSkill(Skill->Code, Skill);
    ...
    Skill->Sum = -40.f;
    Skill->bUse = false;
    Skill->pTex = CResMgr::GetInst()->Load<CTexture>(L"Fire2", L"Texture\\Skill\\Fire2.png").GetPointer();
    CSkillMgr::GetInst()->AddSkill(Skill->Code, Skill);
...
void CSceneMgr::ChangeLobbyTeamState(int blue[4], int red[4])
{
    ...
    for (auto button : m_pCurScene->FindLayer(L"UI")->GetParentObj())
    {
        ...
        if (button->GetChild().size() != 0) {
            for (auto child : button->GetChild()) {
                child->GetScript<CButtonScript>()->SetPlayerInit(Network::GetInst()->getMainClient().id);
            }
        }
    }
...
void CSceneMgr::net_damageMinion(int id, int camp, int hp)
{
    if (CAMP_STATE::BLUE == (CAMP_STATE)camp) {
        for (auto cl : m_pCurScene->FindLayer(L"Blue")->GetParentObj()) {
            if (cl->GetScript<CMinionScript>() != nullptr) {
                if (cl->GetScript<CMinionScript>()->m_GetId() == id) {
                    cl->GetScript<CMinionScript>()->SetHp(hp);
                    break;
                }
            }
        }
    }
    else if (CAMP_STATE::RED == (CAMP_STATE)camp) {
        for (auto cl : m_pCurScene->FindLayer(L"Red")->GetParentObj()) {
            if (cl->GetScript<CMinionScript>() != nullptr) {
                if (cl->GetScript<CMinionScript>()->m_GetId() == id) {
                    cl->GetScript<CMinionScript>()->SetHp(hp);
                    break;
...
void CSceneMgr::net_damageTower(int id, int camp, int hp)
{
    if (CAMP_STATE::BLUE == (CAMP_STATE)camp) {
        for (auto cl : m_pCurScene->FindLayer(L"Blue")->GetParentObj()) {
