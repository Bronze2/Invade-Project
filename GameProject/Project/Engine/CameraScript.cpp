...
void CCameraScript::Update()
{
	...
	CGameObject* pPlayer = dynamic_cast<CGameObject*>(pCurScene->FindLayer(L"Monster")->GetParentObj()[0]);
	...
	case CAMERA_EFFECT_TYPE::ZOOMIN:
		vPos = CameraZoom(vPos);			// 현재 시간으로 관리, 추후에 화살 차징 MAX와 연계하여 변경할 예정
		break;
