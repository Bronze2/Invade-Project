...
void VectorPermute(uint PermuteX, uint PermuteY, uint PermuteZ, uint PermuteW, in float4 V1, in float4 V2,out float4 _vOut)
{
    float4 aPtr[2] = { V1, V2 };
...
float4 VectorShiftLeft(in float4 _V1, in float4 _V2, uint _Element)
{
    float4 vOut = (float4) 0.f;
    VectorPermute(_Element, ((_Element) + 1), ((_Element) + 2), ((_Element) + 3), _V1, _V2, vOut);
